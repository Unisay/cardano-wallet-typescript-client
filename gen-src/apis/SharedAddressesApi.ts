 ...
 * no description
 */
export class SharedAddressesApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async listSharedAddresses(walletId: string, state?: 'used' | 'unused', _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'walletId' is not null or undefined
        if (walletId === null || walletId === undefined) {
            throw new RequiredError("SharedAddressesApi", "listSharedAddresses", "walletId");
        }



        // Path Params
        const localVarPath = '/shared-wallets/{walletId}/addresses'
...
export class SharedAddressesApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to listSharedAddresses
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async listSharedAddresses(response: ResponseContext): Promise<Array<ListAddresses200ResponseInner> > {
